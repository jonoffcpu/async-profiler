...
JfrMetadata::JfrMetadata() : Element("root") {
    *this
        << (element("metadata")
            ...
            << (type("profiler.SignalSample", T_SIGNAL_SAMPLE, "External Signal Sample")
                << category("Java Virtual Machine", "Profiling")
                << field("startTime", T_LONG, "Start Time", F_TIME_TICKS)
                << field("eventThread", T_THREAD, "Thread", F_CPOOL)
                << field("stackTrace", T_STACK_TRACE, "Stack Trace", F_CPOOL)
                << field("correlationId", T_LONG, "Correlation ID", F_UNSIGNED)
                << field("monotonicTimeNanos", T_LONG, "Monotonic Time"))

            << (type("profiler.SignalCapture", T_SIGNAL_CAPTURE, "Signal Capture Context")
                << category("Profiler")
                << field("startTime", T_LONG, "Start Time", F_TIME_TICKS)
                << field("schemaVersion", T_INT, "Schema Version")
                << field("sessionId", T_STRING, "Session ID")
                << field("captureEpoch", T_LONG, "Capture Epoch", F_UNSIGNED)
                << field("signal", T_INT, "Signal")
                << field("signalDelivery", T_STRING, "Signal Delivery")
                << field("processId", T_LONG, "Process ID")
                << field("processStartTimeMillis", T_LONG, "Process Start Time", F_TIME_MILLIS))

            << (type("profiler.SignalCaptureStats", T_SIGNAL_CAPTURE_STATS, "Signal Capture Statistics")
                << category("Profiler")
                << field("startTime", T_LONG, "Start Time", F_TIME_TICKS)
                << field("schemaVersion", T_INT, "Schema Version")
                << field("sessionId", T_STRING, "Session ID")
                << field("captureEpoch", T_LONG, "Capture Epoch", F_UNSIGNED)
                << field("admittedSignals", T_LONG, "Admitted Signals", F_UNSIGNED)
                << field("invalidSignalCode", T_LONG, "Invalid Signal Code", F_UNSIGNED)
                << field("zeroCookie", T_LONG, "Zero Cookie", F_UNSIGNED)
                << field("zeroSequence", T_LONG, "Zero Sequence", F_UNSIGNED)
                << field("staleEpoch", T_LONG, "Stale Epoch", F_UNSIGNED)
                << field("acceptedCookies", T_LONG, "Accepted Cookies", F_UNSIGNED)
                << field("captureFailures", T_LONG, "Capture Failures", F_UNSIGNED)
                << field("submittedSamples", T_LONG, "Submitted Samples", F_UNSIGNED))
